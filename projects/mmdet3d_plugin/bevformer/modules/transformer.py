...
class PerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
        ...
        bev_queries = bev_queries.unsqueeze(1).repeat(1, bs, 1) # (40000, 256) -> (40000, 1, 256) -> (40000, 1, 256), 按batch复制等份的bev query
        bev_pos = bev_pos.flatten(2).permute(2, 0, 1) # (1, 256, 200, 200) -> (1, 256, 40000) -> (40000, 1, 256) 位置编码处理成和bev query相同的维度
        ...
        grid_length_y = grid_length[0] # 2.048 = 102.4 点云范围 / 200 bev网格数
        grid_length_x = grid_length[1] # 2.048

        # 前一帧BEV和当前帧BEV之间对准所需的平移量
        ...
        # 前一帧BEV和当前帧BEV之间对准所需的旋转量
        if prev_bev is not None: #如果存在历史bev，将其旋转到当前bev朝向
            if prev_bev.shape[1] == bev_h * bev_w: # (1, 40000, 256) -> (40000, 1, 256)
            ...
            if self.rotate_prev_bev:
                for i in range(bs): # 按batch遍历
                    ...
                    tmp_prev_bev = prev_bev[:, i].reshape(
                        bev_h, bev_w, -1).permute(2, 0, 1) # (40000, 256) -> (200, 200, 256) -> (256, 200, 200)
                    tmp_prev_bev = rotate(tmp_prev_bev, rotation_angle,
                                          center=self.rotate_center) # bev角度对准, (256, 200, 200)
                    tmp_prev_bev = tmp_prev_bev.permute(1, 2, 0).reshape(
                        bev_h * bev_w, 1, -1) # (256, 200, 200) -> (200, 200, 256) -> (40000, 1, 256)
        ...
        bev_queries = bev_queries + can_bus * self.use_can_bus # (40000, 1, 256) bev query增加canbus信息

        feat_flatten = []
        spatial_shapes = []
        for lvl, feat in enumerate(mlvl_feats): # 遍历多尺度特征
            bs, num_cam, c, h, w = feat.shape # 1, 6, 256, H, W
            spatial_shape = (h, w)
            feat = feat.flatten(3).permute(1, 0, 3, 2) # (1, 6, 256, H*W) -> (6, 1, H*W, 256)
            if self.use_cams_embeds: # 使用相机embedding self.cams_embeds, (6, 256)
                feat = feat + self.cams_embeds[:, None, None, :].to(feat.dtype) # (6, 1, H*W, 256) + (6, 1, 1, 256) -> (6, 1, H*W, 256)
            # self.level_embeds: (4, 256) 使用层级level embedding
            feat = feat + self.level_embeds[None, None, lvl:lvl + 1, :].to(feat.dtype) # (6, 1, H*W, 256) + (1, 1, 1, 256) -> (6, 1, H*W, 256)
            spatial_shapes.append(spatial_shape)
            feat_flatten.append(feat)

        feat_flatten = torch.cat(feat_flatten, 2) # 特征图拼接, (6, 1, 19560, 256)
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=bev_pos.device) # [[92, 160], [46, 80], [23, 40], [12, 20]]
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1,)), spatial_shapes.prod(1).cumsum(0)[:-1])) # 不同层级特征开始位置的索引


        feat_flatten = feat_flatten.permute(
            0, 2, 1, 3)  # (num_cam, H*W, bs, embed_dims) (6, 1, 19560, 256) -> (6, 19560, 1, 256)

        # 将图像特征编码到bev空间，算法核心encoder部分 (temporal attention + spatial attention)
        output = self.encoder(
            bev_queries, # (40000, 1, 256)
            feat_flatten, # (6, 19560, 1, 256)
            feat_flatten, # (6, 19560, 1, 256)
            bev_h=bev_h, # 200
            bev_w=bev_w, # 200
            ...
            prev_bev=prev_bev, # None or (40000, 1, 256)
            shift=shift, # (1, 2)
            # prev_height=prev_height,
            **kwargs
        ) # (1, 40000, 256), (6, 1, 40000, 6)
        bev_embed, height_distribution = output
        return bev_embed, height_distribution
    ...
    def forward(self,
        ...
        output = self.get_bev_features(
            mlvl_feats, # (1, 6, 256, H, W)
            bev_queries, # (40000, 256)
            bev_h, # 200
            bev_w, # 200
            grid_length=grid_length, # 2.048
            bev_pos=bev_pos, # (1, 256, 200, 200)
            prev_bev=prev_bev, # (1, 40000, 256)
            # prev_height=prev_height,
            **kwargs)  # bev_embed shape: bs, bev_h*bev_w, embed_dims, 1, 40000, 256
        bev_embed, height_distribution = output

        bs = mlvl_feats[0].size(0) # 1
        query_pos, query = torch.split(
            object_query_embed, self.embed_dims, dim=1) # (900, 256), (900, 256) object queries
        ...
        reference_points = self.reference_points(query_pos) # 线性层 (1, 900, 256) -> (1, 900, 3) 从query_pos获得reference points
        ...
        bev_embed = bev_embed.permute(1, 0, 2) # (1, 40000, 256) -> (40000, 1, 256)

        inter_states, inter_references = self.decoder(
            query=query, # object query, (900, 1, 256)
            key=None,
            value=bev_embed, # (40000, 1, 256)
        ...
        """
        return bev_embed, height_distribution, inter_states, init_reference_out, inter_references_out
